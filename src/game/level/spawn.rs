...
pub fn spawn_level_from_graph(
    ...
    for (i, node) in graph.nodes.iter().enumerate() {
        ...
        match node.platform_type {
            PlatformType::Start => {
                spawn_start_platform(commands, asset_server, node, layout, season);
            }
            PlatformType::Goal => {
...
fn spawn_other_terrain_objects(
    ...
    for terrain in &node.terrain_objects {
        // Calculate grid position at center of platform, 1 tile above surface
        let center_x = layout.grid_x + layout.width_tiles / 2;
        let object_y = layout.grid_y + layout.height_tiles + 1;
        let grid_pos = GridPosition::primary(center_x, object_y);
        ...
        match terrain {
            ...
            SmartTerrain::SnowSource => {
                spawn_snow(commands, asset_server, grid_pos);
            }
            SmartTerrain::BlockingFire { extinguished } => {
                ...
                spawn_fire(commands, asset_server, grid_pos, fire_state);

                // Spawn blocking wall above fire (only if not extinguished)
                if !extinguished {
                    spawn_fire_wall(commands, grid_pos);
                }
            }
            ...
            SmartTerrain::GoalContainer { .. } => {
                spawn_container(commands, asset_server, grid_pos, ContainerState::Empty);
            }
            ...
            SmartTerrain::Switch { .. } => {
                ...
                info!("Switch terrain not yet implemented at grid {:?}", grid_pos);
            }
            SmartTerrain::MovingPlatform { .. } => {
                // TODO: Implement moving platform
                info!("Moving platform not yet implemented at grid {:?}", grid_pos);
            }
        }
    }
}
...
fn spawn_start_platform(
    ...
    // In Winter, spawn snow + fire instead of water
    let is_winter = matches!(season, Season::Winter);

    // Generate resource layout: 1-2 tiles wide, starting right next to the left wall
    let resource_width = rng.random_range(1..=2);
    let resource_start_x = platform_x_tiles + 2; // Start right at the platform edge (next to wall)
    ...
    for y_offset in 0..layout.height_tiles {
        for x_offset in 0..platform_width_tiles {
            ...
            // Skip top layer where water will be (except in winter)
            if !is_winter
                && y_offset == layout.height_tiles - 1
                && tile_x >= resource_start_x
                && tile_x < resource_start_x + resource_width + 1
            {
                continue;
            }
    ...
    let resource_y = platform_y_tiles + layout.height_tiles;

    if is_winter {
        // Winter: Spawn fire and snow instead of water
        // Fire is on the platform surface at resource_start_x + 1
        ...
        for x_offset in 0..=resource_width + 2 {
            let z = if x_offset <= 0 || x_offset > resource_width + 1 {
                -0.1
            } else {
                20.
            };
            let tile_x = resource_start_x + x_offset;
            let water_grid_pos = GridPosition::primary(tile_x, resource_y);
        ...
        let waterfall_x = resource_start_x + 1;
        ...
        spawn_water(
            commands,
            asset_server,
            GridPosition::primary(waterfall_x, resource_y + 1),
        ...
        for i in 0..waterfall_middle_count {
            spawn_water(
                commands,
                asset_server,
                GridPosition::primary(waterfall_x, resource_y + 2 + i),
        ...
        spawn_water(
            commands,
            asset_server,
            GridPosition::primary(waterfall_x, resource_y + 2 + waterfall_middle_count),
...
fn spawn_fire_wall(commands: &mut Commands, fire_grid_pos: GridPosition) {
...
pub fn update_player_spawn_point(
    ...
    if let Some(start_layout) = layouts.get(&graph.start) {
        // Position player 7 tiles to the right of the left edge of the platform
        let spawn_x = (start_layout.grid_x + 7) as f32 * TILE_SIZE;
